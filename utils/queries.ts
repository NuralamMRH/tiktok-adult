export const getAllPostsByLimit = (userId: string, page = 1, limit = 10) => {
  ...
  | order(_createdAt desc)[${start}...${end}] {
    ...
    "totalLikes": coalesce(likeCount, count(likes)),
    comments[]{
      comment,
      _key,
      postedBy->{
...
export const captionPostsQuery = (
  caption: string | string[],
  userId: string,
) => {
  const query = `*[_type == "post" && caption == '${caption}'] | order(_createdAt desc) [1...10] {
    _id,
     caption,
       video{
        asset->{
...
export const allCaptionsQuery = () => {
  const query = `*[_type == "post" && defined(caption) && caption != ""]{ caption }`;
  return query;
};
